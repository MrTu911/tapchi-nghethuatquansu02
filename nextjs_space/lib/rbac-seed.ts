/**
 * RBAC Seed — nguồn sự thật DUY NHẤT cho bộ quyền hệ thống và ma trận phân quyền
 * theo từng vai trò của Tạp chí Nghệ thuật Quân sự Việt Nam.
 *
 * Được dùng chung bởi:
 *  - API route: app/api/permissions/seed/route.ts (nút "Seed" trên dashboard)
 *  - CLI script: scripts/seed-permissions.ts (seed từ terminal, idempotent)
 *
 * Không định nghĩa lại danh sách quyền / ma trận ở nơi khác.
 */

import { PermissionCategory, Role, PrismaClient } from '@prisma/client'
...
export type SeedableRole = Exclude<Role, 'SYSADMIN'>

export const ROLE_PERMISSION_MATRIX: Record<SeedableRole, string[]> = {
  ...
  // Thư ký tòa soạn (Managing Editor): quyền quản lý toàn bộ workflow
  MANAGING_EDITOR: [
    // Bài nộp & bài báo
...
async function grantByCode(db: PrismaLike, role: Role, codes: string[]): Promise<number> {
...
async function grantAllToRole(db: PrismaLike, role: Role): Promise<number> {
  const perms = await db.permission.findMany({ where: { isActive: true } })
  for (const p of perms) {
    await db.rolePermission.upsert({
      where: { role_permissionId: { role, permissionId: p.id } },
      update: { isGranted: true },
...
/**
 * Seed toàn bộ quyền và áp ma trận phân quyền cho mọi vai trò.
 * Idempotent: chạy nhiều lần không tạo bản ghi trùng (upsert theo code/role).
 *
 * @param resetGrants  true = xóa sạch phân quyền hiện có rồi áp lại ma trận mặc định.
 *                     false = chỉ thêm quyền mới, giữ nguyên tùy chỉnh đã có.
 */
export async function seedPermissions(
  db: PrismaLike,
  { resetGrants = false }: { resetGrants?: boolean } = {}
): Promise<SeedPermissionsResult> {
  // 1. Upsert permissions — thêm mới, bổ sung description nếu thiếu
  let added = 0
  ...
  const total = await db.permission.count({ where: { isActive: true } })
  ...
  for (const [role, codes] of Object.entries(ROLE_PERMISSION_MATRIX)) {
    grants[role] = await grantByCode(db, role as Role, codes)
  }
  grants['SYSADMIN'] = await grantAllToRole(db, 'SYSADMIN' as Role)

  return { added, total, resetGrants, grants }
}
